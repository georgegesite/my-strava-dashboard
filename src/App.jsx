...
const STRAVA_API_URL = 'https://www.strava.com/api/v3/athlete/activities?per_page=1'
        ...
        <article className="w-full rounded-3xl border border-slate-800 bg-slate-900/90 p-8 shadow-2xl shadow-cyan-950/30 backdrop-blur md:p-10">
          ...
          {!loading && activity && (
            <div className="mt-8 grid gap-6 rounded-3xl border border-cyan-500/20 bg-linear-to-br from-slate-800 to-slate-900 p-6 md:grid-cols-[1.1fr_0.9fr] md:p-8">
              <div>
                <p className="text-sm uppercase tracking-[0.3em] text-cyan-300">Most recent ride</p>
                <h2 className="mt-3 text-3xl font-semibold text-white">{activity.name}</h2>
                <p className="mt-2 text-slate-300">{formatDate(activity.start_date_local)} • {activity.sport_type}</p>
              ...
              <div className="grid gap-4 rounded-2xl border border-slate-700 bg-slate-950/70 p-5 text-slate-100">
                <div>
                  ...
                  <p className="mt-1 text-3xl font-semibold text-cyan-300">{formatDistance(activity.distance)}</p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.35em] text-slate-400">Moving time</p>
                  <p className="mt-1 text-3xl font-semibold text-cyan-300">{formatDuration(activity.moving_time)}</p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.35em] text-slate-400">Calories</p>
                  <p className="mt-1 text-3xl font-semibold text-cyan-300">{activity.calories ?? '—'}</p>
                </div>
              </div>
            </div>
          )}
